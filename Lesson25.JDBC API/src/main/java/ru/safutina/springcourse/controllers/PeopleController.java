...
public class PeopleController {
    ...
    public String create(@ModelAttribute("person") @Valid Person person, BindingResult bindingResult) {
        if (bindingResult.hasErrors())
            return "people/new";
        /*
        Эта форма будет с ошибками, они будут показываться с помощью Thymeleaf.
        Так как в объекте person уже содержаться ошибки, которые получились в результате валидации.
        Так как аннотация ModelAttribute автоматически добавляет объект в модель, здесь, когда мы вернем
        эту форму, на этой форме будет этот объект класса Person с ошибками, коорые внедрились автоматически
        с помощью аннотации Valid!
         */
    ...
    public String edit(Model model, @PathVariable("id") int id) {
        //9model.addAttribute("person", personDAO.show(id));
        return "people/edit";
    }
